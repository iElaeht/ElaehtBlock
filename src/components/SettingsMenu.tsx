...
export const SettingsMenu = ({
  ...
  // Optimizamos las clases del overlay para evitar re-calculos pesados
  const overlayClasses = useMemo(() => `
    fixed inset-0 z-[500] flex items-end justify-center transition-all duration-500 transform-gpu
    ${performanceMode ? "bg-black/95 backdrop-blur-none" : "bg-black/60 backdrop-blur-md"}
  `, [performanceMode]);
  ...
  return (
    ...
    >
      <motion.div 
        ...
        animate={{ y: 0 }}
        exit={{ y: "100%" }}
        transition={{ type: "spring", damping: 30, stiffness: 250 }} // Ajuste de fluidez
        drag="y"
        dragConstraints={{ top: 0 }}
        dragElastic={0.05}
        onDragEnd={(_, info) => { if (info.offset.y > 80) onClose(); }}
        // will-change-transform ayuda al hardware del móvil a mover el panel suavemente
        className="w-full max-w-[500px] bg-[#0f0f0f] rounded-t-[3rem] p-8 pb-10 shadow-2xl relative border-t border-white/10 will-change-transform"
      >
          ...
          {!isConfirmingDelete ? (
            ...
            >
              ...
              <div className="w-full bg-white/5 rounded-[2.5rem] overflow-hidden mb-10 border border-white/5">
                <button onClick={onToggleMute} className="w-full p-6 flex justify-between items-center active:bg-white/10 border-b border-white/5 transition-colors transform-gpu">
                  <span className="text-white/60 text-[10px] font-black uppercase tracking-widest">Sonido Global</span>
                  <div className="flex gap-2">
                    <span className={`font-black text-xs transition-colors ${!isMuted ? 'text-blue-400' : 'text-white/10'}`}>ON</span>
                    <span className="text-white/10 font-black text-xs">/</span>
                    <span className={`font-black text-xs transition-colors ${isMuted ? 'text-red-500' : 'text-white/10'}`}>OFF</span>
                  </div>
                </button>
                
                <button onClick={onTogglePerformance} className="w-full p-6 flex justify-between items-center active:bg-white/10 transition-colors transform-gpu">
                  <span className="text-white/60 text-[10px] font-black uppercase tracking-widest">Rendimiento Modo</span>
                  <div className="flex items-center gap-3">
                    <div className="flex gap-2 items-center">
                      <span className={`font-black text-xs transition-colors ${performanceMode ? 'text-emerald-500' : 'text-white/10'}`}>ON</span>
                      <span className="text-white/10 font-black text-xs">/</span>
                      <span className={`font-black text-xs transition-colors ${!performanceMode ? 'text-blue-400' : 'text-white/10'}`}>OFF</span>
                    </div>
                    <span className={`text-[8px] font-black px-2 py-0.5 rounded border transition-colors ${performanceMode ? 'border-emerald-500/50 text-emerald-500' : 'border-blue-400/50 text-blue-400'}`}>
              ...
              <div className="w-full flex flex-col gap-4">
                <button onClick={onResetGame} className="w-full py-5 bg-white/5 border border-white/10 text-white rounded-[1.5rem] font-black text-xs uppercase tracking-[0.4em] active:scale-[0.97] transition-all transform-gpu">
                  Reiniciar Partida
                </button>
                <button onClick={onExit} className="w-full py-5 bg-white text-black rounded-[1.5rem] font-black text-xs uppercase tracking-[0.4em] shadow-xl active:scale-[0.97] transition-all transform-gpu">
                  Volver al Inicio
                </button>
                <button onClick={() => setIsConfirmingDelete(true)} className="w-full py-3 text-red-500/80 hover:text-red-500 font-black text-[9px] uppercase tracking-[0.3em] transition-colors">
